...
class DeviceUUID {
  ...
  constructor(userAgent: string, options: DeviceUUIDOptions = {}) {
    ...
    const defOptions: DeviceUUIDOptions = {
      version: false,
      language: false,
      platform: true,
      os: true,
    ...
    for (const key of Object.keys(defOptions) as (keyof DeviceUUIDOptions)[]) {
      if (defOptions[key] !== undefined && options[key] !== undefined) {
    ...
    this.DefaultAgent = {
      ...
      geoIp: {},
      source: '',
      hashInt: function (string: string): number {
        let hash = 0,
          i,
          chr,
          len;
      ...
      hashMD5: function (string: string): string {
        ...
        function gFF(a: number, b: number, c: number, d: number, x: number, s: number, ac: number) {
          a = addUnsigned(a, addUnsigned(addUnsigned(gF(b, c, d), x), ac));
          return addUnsigned(rotateLeft(a, s), b);
        }

        function gGG(a: number, b: number, c: number, d: number, x: number, s: number, ac: number) {
          a = addUnsigned(a, addUnsigned(addUnsigned(gG(b, c, d), x), ac));
          return addUnsigned(rotateLeft(a, s), b);
        }

        function gHH(a: number, b: number, c: number, d: number, x: number, s: number, ac: number) {
          a = addUnsigned(a, addUnsigned(addUnsigned(gH(b, c, d), x), ac));
          return addUnsigned(rotateLeft(a, s), b);
        }

        function gII(a: number, b: number, c: number, d: number, x: number, s: number, ac: number) {
    ...
    if (this.Agent.isIE) {
      if (/Trident\/(\d)\.0/i.test(ua.Agent.source)) {
        ...
        if (version === 7 && tridentVersion === 7) {
          ...
          ua.Agent.version = (11.0).toString();
        }

        if (version === 7 && tridentVersion === 6) {
          ua.Agent.isIECompatibilityMode = true;
          ua.Agent.version = (10.0).toString();
        }

        if (version === 7 && tridentVersion === 5) {
          ua.Agent.isIECompatibilityMode = true;
          ua.Agent.version = (9.0).toString();
        }

        if (version === 7 && tridentVersion === 4) {
          ua.Agent.isIECompatibilityMode = true;
          ua.Agent.version = (8.0).toString();
        }
      }
    }
  }
  ...
  testNginxGeoIP(headers: { [key: string]: string }) {
    const ua = this;
  ...
  reset() {
    ...
    ua.Agent = ua.DefaultAgent;

    return ua;
  }

  parse(source: string = this.userAgent) {
    const ua = new DeviceUUID(source);
  ...
  get(customData?: string) {
    let pref = 'a';
    const du = this.parse();
    const dua: (string | number | boolean | number[])[] = [];
    for (const key of Object.keys(this.options) as (keyof DeviceUUIDOptions)[]) {
...
export default new DeviceUUID(navigator.userAgent);
